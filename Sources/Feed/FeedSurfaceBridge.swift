...
final class FeedSurfaceBridge: NSObject, WKScriptMessageHandlerWithReply {
    ...
    static let shared = FeedSurfaceBridge()
    ...
    static func isTrustedFeedURL(_ url: URL?, resourceURL: URL? = Bundle.main.resourceURL) -> Bool {
        guard let url, let expected = feedHTMLURL(resourceURL: resourceURL) else { return false }
        return normalizedFileURL(url) == normalizedFileURL(expected)
    }

    static func feedHTMLURL(resourceURL: URL? = Bundle.main.resourceURL) -> URL? {
        resourceURL?
            .appendingPathComponent("markdown-viewer", isDirectory: true)
            .appendingPathComponent("webviews-app", isDirectory: true)
            .appendingPathComponent("feed.html", isDirectory: false)
    }

    private static func normalizedFileURL(_ url: URL) -> URL? {
        guard url.isFileURL else { return nil }
        var components = URLComponents(url: url.standardizedFileURL.resolvingSymlinksInPath(), resolvingAgainstBaseURL: false)
